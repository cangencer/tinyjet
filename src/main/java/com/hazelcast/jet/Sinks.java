...
import com.hazelcast.jet.impl.connector.WriteListP;
...
public final class Sinks {
    ...
    public static <K, V, E extends Map.Entry<K, V>> Sink<E> map(Map<K, V> map) {
        ProcessorMetaSupplier pms = ProcessorMetaSupplier.dontParallelize(() -> new WriteMapP<>(map));
        return fromProcessor(System.identityHashCode(map) + "", pms);
    }

    public static <T> Sink<T> list(List<T> list) {
        ProcessorMetaSupplier pms = ProcessorMetaSupplier.dontParallelize(() -> new WriteListP<>(list));
        return fromProcessor(System.identityHashCode(list) + "", pms);
    }
