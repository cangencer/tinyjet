...
public class TaskletExecutionService {
    ...
    public TaskletExecutionService(int threadCount) {
    ...
    public CompletableFuture<Void> beginExecute(
        ...
        try {
            ...
            submitCooperativeTasklets(executionTracker, byCooperation.get(true));
            submitBlockingTasklets(executionTracker, byCooperation.get(false));
        } catch (Throwable t) {
    ...
    private void submitBlockingTasklets(ExecutionTracker executionTracker,
        ...
        executionTracker.blockingFutures = tasklets
                .stream()
                .map(t -> new BlockingWorker(new TaskletTracker(t, executionTracker), startedLatch))
    ...
    private void submitCooperativeTasklets(
            ExecutionTracker executionTracker, List<Tasklet> tasklets
    ) {
        ...
        for (Tasklet t : tasklets) {
            ...
            trackersByThread[cooperativeThreadIndex.getAndUpdate(i -> (i + 1) % trackersByThread.length)]
                    .add(new TaskletTracker(t, executionTracker));
        }
    ...
    private synchronized void ensureThreadsStarted() {
        ...
        Arrays.setAll(cooperativeThreadPool, i -> {
            Thread t = new Thread(cooperativeWorkers[i],
                    String.format("hjl.cooperative.thread-%d", i));
            t.setDaemon(true);
            return t;
        });
...
//        final ClassLoader jobClassLoader;
        ...
        TaskletTracker(Tasklet tasklet, ExecutionTracker executionTracker) {
    ...
    private final class BlockingTaskThreadFactory implements ThreadFactory {
        ...
        public Thread newThread(@Nonnull Runnable r) {
            Thread t = new Thread(r,
                    String.format("hjl.blocking.thread-%d", seq.getAndIncrement()));
            t.setDaemon(true);
            return t;
        }
    }
